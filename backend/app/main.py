...
async def get_json(client:httpx.AsyncClient,url:str,**kwargs:Any)->Any:
    response=await client.get(url,**kwargs);response.raise_for_status();return response.json()

async def safe_json(client:httpx.AsyncClient,url:str,**kwargs:Any)->tuple[Any|None,str|None]:
    try:return await get_json(client,url,**kwargs),None
    except (httpx.HTTPError,ValueError) as exc:return None,str(exc)

def normalize_feature(feature:dict[str,Any])->dict[str,Any]|None:
    geometry=feature.get("geometry") or {};coordinates=geometry.get("coordinates") or [];properties=feature.get("properties") or {}
    if len(coordinates)<2 or properties.get("mag") is None:return None
...
@app.get("/api/health")
async def health()->dict[str,str]:return {"status":"online","service":"world-monitor"}
...
async def earthquakes(hours:int=Query(default=24,ge=1,le=168),min_magnitude:float=Query(default=2.5,ge=-1,le=10))->dict[str,Any]:
    try:
        async with httpx.AsyncClient(timeout=12) as client:
            if hours<=24 and min_magnitude<=2.5:payload=await get_json(client,USGS_FEED)
            else:
                start=datetime.now(timezone.utc)-timedelta(hours=hours);payload=await get_json(client,USGS_QUERY,params={"format":"geojson","starttime":start.isoformat(),"endtime":datetime.now(timezone.utc).isoformat(),"minmagnitude":min_magnitude,"orderby":"time","limit":2000})
    except httpx.HTTPError as exc:raise HTTPException(status_code=502,detail=f"USGS request failed: {exc}") from exc
    events=[item for feature in payload.get("features",[]) if (item:=normalize_feature(feature))];events.sort(key=lambda event:event.get("timestamp") or "",reverse=True)
...
async def flights()->dict[str,Any]:
    try:
        async with httpx.AsyncClient(timeout=15) as client:payload=await get_json(client,OPENSKY_STATES)
    except httpx.HTTPError as exc:raise HTTPException(status_code=502,detail=f"OpenSky request failed: {exc}") from exc
    states=[]
    for row in payload.get("states") or []:
        if len(row)<11 or row[5] is None or row[6] is None:continue
...
@app.get("/api/ships")
async def ships()->dict[str,Any]:return {"source":"AIS","status":"unavailable","message":"Live global AIS requires a vessel-data provider/API key. No fabricated vessel positions are shown.","ships":[]}

@app.get("/api/iss")
async def iss()->dict[str,Any]:
    try:
        async with httpx.AsyncClient(timeout=10) as client:data=await get_json(client,ISS_API)
    except httpx.HTTPError as exc:raise HTTPException(status_code=502,detail=f"ISS request failed: {exc}") from exc
...
async def currency(base:str=Query(default="USD",min_length=3,max_length=3),symbols:str=Query(default="EUR,INR,GBP,JPY",max_length=100))->dict[str,Any]:
    try:
        async with httpx.AsyncClient(timeout=10) as client:data=await get_json(client,FRANKFURTER,params={"from":base.upper(),"to":symbols.upper()})
    except httpx.HTTPError as exc:raise HTTPException(status_code=502,detail=f"Currency request failed: {exc}") from exc
    return {"source":"Frankfurter",**data}

@app.get("/api/news")
async def news(query:str=Query(default="world",min_length=2,max_length=100))->dict[str,Any]:
    try:
        async with httpx.AsyncClient(timeout=15) as client:data=await get_json(client,GDELT,params={"query":query,"mode":"artlist","format":"json","maxrecords":12,"sort":"datedesc"})
    except httpx.HTTPError as exc:raise HTTPException(status_code=502,detail=f"GDELT request failed: {exc}") from exc
...
async def space()->dict[str,Any]:
    try:
        async with httpx.AsyncClient(timeout=15) as client:data=await get_json(client,NASA_DONKI+"/FLR",params={"startDate":(datetime.now(timezone.utc)-timedelta(days=3)).date().isoformat(),"endDate":datetime.now(timezone.utc).date().isoformat(),"api_key":"DEMO_KEY"})
    except httpx.HTTPError as exc:raise HTTPException(status_code=502,detail=f"NASA DONKI request failed: {exc}") from exc
...
async def dns(host:str=Query(min_length=1,max_length=253),record_type:str=Query(default="A",max_length=10))->dict[str,Any]:
    try:
        async with httpx.AsyncClient(timeout=10) as client:data=await get_json(client,DNS_GOOGLE,params={"name":host,"type":record_type.upper()})
    except httpx.HTTPError as exc:raise HTTPException(status_code=502,detail=f"DNS lookup failed: {exc}") from exc
...
async def location(name:str=Query(min_length=2,max_length=100))->dict[str,Any]:
    ...
    except (httpx.HTTPError,ValueError,KeyError) as exc:raise HTTPException(status_code=502,detail=f"Location intelligence request failed: {exc}") from exc
    weather_payload,weather_error=weather_data;quake_payload,quake_error=quake_data;news_payload,news_error=news_data
...
async def command(q:str=Query(min_length=2,max_length=200))->dict[str,Any]:
    text=q.strip();lower=text.lower();patterns={"earthquakes":("earthquake","earthquakes","seismic","quake"),"weather":("weather","temperature","forecast","climate"),"flights":("flight","flights","aircraft","plane"),"ships":("ship","ships","vessel","marine"),"iss":("iss","space station","international space station"),"currency":("currency","exchange rate","forex"),"news":("news","headlines"),"space":("space","solar","satellite","nasa"),"network":("ip","dns","network","asn")}
    for intent,words in patterns.items():
        if any(word in lower for word in words):
            if intent=="weather":
                match=re.search(r"(?:weather|temperature|forecast|climate)\s+(?:in|at|for)?\s*(.+)$",text,re.I);city=(match.group(1).strip(" ?.,") if match else "Mumbai") or "Mumbai";return {"intent":intent,"city":city,"message":f"Weather channel selected for {city}."}
            if intent in {"news","earthquakes"}:
                match=re.search(r"(?:news|headlines|earthquakes?|seismic|quakes?)\s+(?:in|at|near|around|for)?\s*(.+)$",text,re.I);place=(match.group(1).strip(" ?.,") if match else "");return {"intent":intent,"place":place,"message":f"{intent.upper()} channel selected{f' for {place}' if place else ''}."}
            return {"intent":intent,"message":f"{intent.upper()} channel selected."}
    return {"intent":"location","location":text,"message":f"Location intelligence selected for {text}."}
