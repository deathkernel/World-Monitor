...
type Countries = {
  ...
  features: { type: 'Feature'; properties?: { name?: string }; geometry: Country }[];
};
...
function Borders({ data }: { data: Countries | null }) {
  ...
  return (
    <lineSegments geometry={geometry}>
      <lineBasicMaterial color="#4fb9d7" transparent opacity={0.52} depthWrite={false} />
    </lineSegments>
  );
}
...
function Signal({ e, selected, onSelect }: { e: Event; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <group position={[x, y, z]} onClick={(event) => { event.stopPropagation(); onSelect(); }}>
      <mesh ref={ring}>
        <ringGeometry args={[0.045, 0.052, 24]} />
        <meshBasicMaterial color={e.mag >= 6 ? '#ff496c' : '#55d6ff'} transparent />
      </mesh>
      <mesh ref={core}>
        <sphereGeometry args={[0.05, 12, 12]} />
        <meshBasicMaterial color={selected ? '#fff' : e.mag >= 6 ? '#ff496c' : '#55d6ff'} />
      </mesh>
    </group>
  );
}
...
function Globe({ events, countries, selected, onSelect, scanning }: { events: Event[]; countries: Countries | null; selected: string | null; onSelect: (e: Event) => void; scanning: boolean }) {
  ...
  return (
    <group ref={group}>
      ...
      {events.map((event) => <Signal key={event.id} e={event} selected={event.id === selected} onSelect={() => onSelect(event)} />)}
      {scanning && <mesh rotation={[Math.PI / 2, 0, 0]}><torusGeometry args={[2.025, 0.006, 8, 96]} /><meshBasicMaterial color="#65dcff" transparent opacity={0.18} /></mesh>}
    </group>
  );
}
...
function App() {
  ...
  const load = useCallback(async (c: string) => {
    ...
    setSelected(null);
    setData(null);
    setStatus(`FETCHING • ${c.toUpperCase()}`);
    try {
      let url = `${API}/api/${c}`;
      if (c === 'weather') url += '?city=Mumbai';
      if (c === 'news') url += '?query=world';
      if (c === 'currency') url += '?base=USD&symbols=EUR,INR,GBP,JPY';
      if (c === 'network') url += '?host=google.com';
      const response = await fetch(url);
      if (!response.ok) throw new Error('Request failed');
      const result = await response.json();
      setData(result);
      setStatus('SYNCHRONIZED • LIVE');
      setMsg(result.status === 'unavailable' ? result.message : `${c.toUpperCase()} intelligence synchronized.`);
      if (c === 'earthquakes') {
        setEvents((result.events || []).map((event: any) => ({ ...event, lat: event.latitude, lon: event.longitude, mag: event.severity })));
      }
    } catch {
  ...
  const run = async (text: string) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.intent === 'weather') await load('weather');
      else if (result.intent === 'earthquakes') await load('earthquakes');
      else if (['flights', 'ships', 'iss', 'currency', 'news', 'space', 'network'].includes(result.intent)) await load(result.intent);
  ...
  const pointCount = channel === 'earthquakes' ? events.length : data?.count ?? data?.aircraft?.length ?? '--';
    ...
    <main className="app">
      ...
      <nav className="channels">
        {channels.map(([id, icon, label]) => (
          <button className={channel === id ? 'active' : ''} onClick={() => load(id)} key={id}><span>{icon}</span>{label}</button>
        ))}
      </nav>
      ...
      <section className="scene">
        ...
        <div className="globe-wrap">
          <Canvas camera={{ position: [0, 0, 6.4], fov: 40 }} dpr={[1, 1.6]}>
            <Globe events={events} countries={countries} selected={selected?.id || null} onSelect={setSelected} scanning={scan} />
          </Canvas>
        </div>

        {selected && channel === 'earthquakes' ? (
          <aside className="event-panel">
            <button className="close-panel" onClick={() => setSelected(null)}>×</button>
            <small>SELECTED SIGNAL</small>
            <h2>{selected.location}</h2>
            <div className="big">M{selected.mag.toFixed(1)}</div>
            <div className="grid">
              <span>LAT<b>{selected.lat.toFixed(2)}°</b></span>
              <span>LON<b>{selected.lon.toFixed(2)}°</b></span>
        ...
        <aside className="data-panel">
          {channel === 'weather' && data?.location ? (
            <div><small>WEATHER / {data.location.name}</small><div className="hero">{Math.round(data.current?.temperature_2m ?? 0)}°</div><div className="grid"><span>FEELS LIKE<b>{data.current?.apparent_temperature}°</b></span><span>HUMIDITY<b>{data.current?.relative_humidity_2m}%</b></span><span>WIND<b>{data.current?.wind_speed_10m} km/h</b></span><span>TIMEZONE<b>{data.location.timezone}</b></span></div></div>
          ) : null}

          {channel === 'flights' ? (
            <div><small>✈ LIVE AIRCRAFT</small><div className="hero">{data?.count ?? '--'}</div><div className="grid"><span>TRACKED<b>OPENSKY</b></span><span>VISIBLE<b>{data?.aircraft?.length ?? 0}</b></span></div></div>
          ) : null}

          {channel === 'ships' ? (
            <div><small>🚢 VESSEL NETWORK</small><div className="hero">{data?.status === 'unavailable' ? 'OFFLINE' : data?.count ?? 0}</div><p>{data?.message}</p></div>
          ) : null}

          {channel === 'iss' && data ? (
            <div><small>🌍 INTERNATIONAL SPACE STATION</small><div className="hero">{Number(data.latitude).toFixed(2)}°</div><div className="grid"><span>LONGITUDE<b>{Number(data.longitude).toFixed(2)}°</b></span><span>ALTITUDE<b>{Number(data.altitude_km).toFixed(1)} km</b></span><span>VELOCITY<b>{Number(data.velocity_kmh).toFixed(0)} km/h</b></span><span>VISIBILITY<b>{data.visibility}</b></span></div></div>
          ) : null}

          {channel === 'currency' && data?.rates ? (
            <div><small>💱 FX MATRIX / USD</small><div className="rate-list">{Object.entries(data.rates).map(([key, value]) => <div key={key}><b>{key}</b><span>{String(value)}</span></div>)}</div></div>
          ) : null}

          {channel === 'news' && data?.articles ? (
            <div><small>📰 GLOBAL HEADLINES</small><div className="news-list">{data.articles.slice(0, 7).map((article: any, index: number) => <div key={index}><b>{article.title}</b><span>{article.domain} · {article.date || 'LIVE'}</span></div>)}</div></div>
          ) : null}

          {channel === 'space' && data ? (
            <div><small>🛰️ SPACE WEATHER / NASA DONKI</small><div className="hero">{Array.isArray(data.events) ? data.events.length : '--'}</div><p>Recent solar events available from NASA DONKI.</p></div>
          ) : null}

          {channel === 'network' && data ? (
            <div><small>🌐 NETWORK INTELLIGENCE</small><div className="hero">{data.ip || '--'}</div><div className="grid"><span>ORG<b>{data.org || '—'}</b></span><span>ASN<b>{data.asn || '—'}</b></span><span>COUNTRY<b>{data.country || '—'}</b></span><span>ZONE<b>{data.timezone || '—'}</b></span></div></div>
          ) : null}

          {channel === 'earthquakes' && !selected ? (
            <div><small>🌋 SEISMIC NETWORK</small><div className="hero">{events.length}</div><div className="grid"><span>WINDOW<b>24 HOURS</b></span><span>SOURCE<b>USGS</b></span></div></div>
          ) : null}
        </aside>
      </section>

      <section className="assistant">
        <div className="assistant-line"><span>◉</span><div><small>{status}</small><p>{msg}</p></div></div>
        <form onSubmit={(event) => { event.preventDefault(); run(q); }}>
          <input value={q} onChange={(event) => setQ(event.target.value)} placeholder="Ask J.A.R.V.I.S. about the world..." />
