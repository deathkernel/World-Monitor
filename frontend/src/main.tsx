...
function Signal({event,selected,onSelect}:{event:WorldEvent;selected:boolean;onSelect:()=>void}){
  ...
  const [x,y,z]=latLon(event.lat,event.lon);
  ...
  return <group position={[x,y,z]} onClick={e=>{e.stopPropagation();onSelect()}}>
    <mesh ref={ring}><ringGeometry args={[0.045,0.052,24]}/><meshBasicMaterial color={event.mag>=6?'#ff496c':'#55d6ff'} transparent/></mesh>
    <mesh ref={core}><sphereGeometry args={[0.045+Math.min(event.mag,9)/180,12,12]}/><meshBasicMaterial color={selected?'#ffffff':event.mag>=6?'#ff496c':'#55d6ff'}/></mesh>
  </group>;
}
function Globe({events,selectedId,onSelect}:{events:WorldEvent[];selectedId:string|null;onSelect:(e:WorldEvent)=>void}){
  ...
  return <group ref={group} rotation={[0.12,0.2,0]}>
...
function App(){
 ...
 useEffect(()=>{let alive=true;const sync=async()=>{try{setStatus('SCANNING • GLOBAL SIGNALS');await loadEarthquakes();if(alive){setStatus('SYNCHRONIZED • LIVE');setMessage('Live seismic network synchronized. Monitoring global activity.');}}catch{if(alive){setStatus('DEGRADED • RETRYING');setMessage('Live core unavailable. Check that FastAPI is running.');}}};sync();const t=window.setInterval(sync,60000);return()=>{alive=false;clearInterval(t);};},[loadEarthquakes]);
 ...
 return <main className="app"><div className="scanline"/><header className="top"><div className="brand">WORLD <span>// LIVE</span></div><div className="top-center"><i/> J.A.R.V.I.S. CORE <b>CONNECTED</b></div><div className="time">GLOBAL NODE • 01</div></header>
 <section className="scene"><div className="reticle"/><div className="hud hud-left"><small>GLOBAL ACTIVITY</small><strong>{events.length.toString().padStart(2,'0')}</strong><span>EARTHQUAKES / 24H</span></div><div className="hud hud-right"><small>HIGH SIGNALS</small><strong>{mag6.toString().padStart(2,'0')}</strong><span>MAG ≥ 6.0 EVENTS</span></div>
 <div className="globe-wrap"><Canvas camera={{position:[0,0,6.4],fov:40}} dpr={[1,1.6]}><ambientLight intensity={1}/><Globe events={events} selectedId={selected?.id||null} onSelect={setSelected}/></Canvas></div>
 <div className="orbit-label one">WEATHER <b>{weather?`${Math.round(weather.current.temperature_2m??0)}°`:'SYNC'}</b></div><div className="orbit-label two">SEISMIC <b>LIVE</b></div><div className="orbit-label three">SYNC <b>{lastSync?lastSync.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):'WAITING'}</b></div>
 {selected&&<aside className="event-panel"><button className="close-panel" onClick={()=>setSelected(null)}>×</button><small>SELECTED SIGNAL</small><h2>{selected.location}</h2><div className="event-magnitude"><span>MAGNITUDE</span><strong>{selected.mag.toFixed(1)}</strong></div><div className="event-grid"><span>LATITUDE <b>{selected.lat.toFixed(2)}°</b></span><span>LONGITUDE <b>{selected.lon.toFixed(2)}°</b></span><span>DEPTH <b>{selected.depth!=null?`${selected.depth.toFixed(1)} km`:'—'}</b></span><span>TSUNAMI <b>{selected.tsunami?'YES':'NO'}</b></span></div><p>{selected.timestamp?new Date(selected.timestamp).toLocaleString():'Timestamp unavailable'} · {selected.source}</p></aside>}</section>
 <section className="assistant"><div className="assistant-line"><span>◉</span><div><small>{status}</small><p>{message}</p></div></div><form onSubmit={e=>{e.preventDefault();run(query)}}><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Ask me about the world..."/><button>EXECUTE</button></form><div className="chips"><button onClick={()=>run('What is happening in Japan?')}>ANALYZE JAPAN</button><button onClick={()=>run('Earthquakes right now')}>SEISMIC ACTIVITY</button><button onClick={()=>run('Weather in Mumbai')}>MUMBAI WEATHER</button></div></section>
 <footer><span>J.A.R.V.I.S. / WORLD INTELLIGENCE</span><span>USGS • OPEN-METEO • FASTAPI CORE</span><span>LIVE REFRESH • 60S ●</span></footer></main>;
}
